...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      // Fetch available notes and categories
      ...
      if (productId) {
        ...
        if (productData && !error) {
          ...
          setMrp(productData.sale_price?.toString() || "");
          setStock(productData.stock_quantity?.toString() || "");
          ...
          if (productData.metadata) {
            ...
            if (m.notes && Array.isArray(m.notes)) {
              // Map saved notes back to DB notes structure
              const mappedNotes = m.notes.map((n: any) => ({
                id: n.name, // using name as id since we don't have the original ID in metadata
                name: n.name,
                image_url: n.image
              }));
              setSelectedNotes(mappedNotes);
            }
            
            if (m.images && Array.isArray(m.images)) {
              const newImages = [null, null, null, null];
              const newPreviews = ["", "", "", ""];
              m.images.forEach((img: string, i: number) => {
                if (i < 4) {
                  newImages[i] = img as any;
                  newPreviews[i] = img;
                }
              });
              setImages(newImages);
            ...
            if (m.accordion) {
              setDescription(m.accordion.description || "");
              setFeelings(m.accordion.feelings || "");
              setOccasions(m.accordion.occasions || "");
              setBehindPerfume(m.accordion.behind_perfume || "");
            }
          }
        }
  ...
  const toggleNote = (note: Note) => {
    const exists = selectedNotes.find(n => n.name === note.name);
    if (exists) {
      setSelectedNotes(selectedNotes.filter(n => n.name !== note.name));
    } else {
  ...
  if (isFetchingProduct) {
    return <div className="py-24 flex justify-center items-center"><Loader2 className="w-8 h-8 animate-spin text-gray-400" /></div>;
  }
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input type="text" value={title} onChange={e => setTitle(e.target.value)} required className="w-full border border-gray-300 rounded-md p-2.5 text-sm focus:ring-black focus:border-black" />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Slug (URL) *</label>
              <input type="text" value={slug} onChange={e => setSlug(e.target.value)} required className="w-full border border-gray-300 rounded-md p-2.5 text-sm focus:ring-black focus:border-black" />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Product Type</label>
              <input type="text" value={type} onChange={e => setType(e.target.value)} className="w-full border border-gray-300 rounded-md p-2.5 text-sm focus:ring-black focus:border-black" />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Badge</label>
              <input type="text" value={badge} onChange={e => setBadge(e.target.value)} className="w-full border border-gray-300 rounded-md p-2.5 text-sm focus:ring-black focus:border-black" />
            </div>
            <div className="space-y-2">
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              ...
              <input type="number" value={mrp} onChange={e => setMrp(e.target.value)} className="w-full border border-gray-300 rounded-md p-2.5 text-sm focus:ring-black focus:border-black" />
            </div>
            <div className="space-y-2">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4 max-h-80 overflow-y-auto p-1">
            {isLoadingNotes ? (
              ...
              filteredNotes.map((note) => {
                const isSelected = !!selectedNotes.find(n => n.name === note.name);
                return (
                  <div 
                    key={note.id} 
